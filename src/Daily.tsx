...
export default class Daily extends Component<{
  ...
  instanceRef() : firebase.database.Reference{
    return this.props.firebaseRef.child('daily').child(this.props.idString);
  } 
  ...
  handleSubmit(e:any) {
    ...
    this.props.firebaseRef.child('unfinished')
                              ...
                              .set(this.props.idString === new Date().toDateString() ? false : null);
  }
...
const dailyWidgets : {
  ...
  'booly' : function(title, thisRef){
    ...
    const value : string = thisRef.state.data[title] == null 
    ...
    return (
        <div className='form-row justify-content-between align-items-center'>
          ...
          <div className='btn-group col-lg-auto align-self-end' data-toggle='buttons'>
            <label className={'btn btn-success ' + value === 'true' ? 'active' : ''} onClick={()=>thisRef.handleRadioClick(title, true)}>
              Yes
            </label>
            <label className={'btn btn-warning ' + value === null ? 'active' : ''}  onClick={()=>thisRef.handleRadioClick(title, null)}>
              Not Sure
            </label>
            <label className={'btn btn-danger ' + value === 'false' ? 'active' : ''} onClick={()=>thisRef.handleRadioClick(title, false)}>
              No
            </label>
          </div>
        </div>
